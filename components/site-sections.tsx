...
export function QuoteLink({ children = 'Get my free quote' }: { children?: React.ReactNode }) {
  return <Link href="/contact" className={`${buttonVariants()} brand-button h-14 px-7 text-base`}>{children}<ArrowUpRight size={18}/></Link>;
}
...
export function ClosingCTA() {
  return <section className="bg-[#fff0c7] py-16 md:py-20"><div className="site-container flex flex-col items-start justify-between gap-8 md:flex-row md:items-center"><div><p className="eyebrow">Your yard. Crime scene clean.</p><h2 className="section-title max-w-2xl">Ready to call in<br/>the cleanup crew?</h2></div><QuoteLink/></div></section>;
}
