...
export function QuoteForm() {
  ...
  return (
    <form action={submit} className="rounded-2xl border bg-card p-6 sm:p-9">
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <div className="space-y-2">
          ...
          <NativeSelect id="dogs" name="dogs" required className="w-full [&_select]:h-12 [&_select]:bg-background">
            <option value="">Choose one</option><option value="1">1 dog</option><option value="2">2 dogs</option><option value="3">3 dogs</option><option value="4+">4 or more</option><option value="shared">Shared / commercial property</option>
          </NativeSelect>
        </div>
        <div className="space-y-2">
          ...
          <NativeSelect id="frequency" name="frequency" required className="w-full [&_select]:h-12 [&_select]:bg-background">
            <option value="">Choose one</option><option value="weekly">Weekly</option><option value="biweekly">Every other week</option><option value="monthly">Monthly</option><option value="onetime">One-time / spring cleanup</option><option value="commercial">Commercial service</option><option value="unsure">Not sure yet</option>
          </NativeSelect>
        </div>
      </div>
      <div className="mt-5 space-y-2"><Label htmlFor="notes">Anything else we should know? <span className="font-normal text-muted-foreground">(optional)</span></Label><Textarea id="notes" name="notes" rows={4} maxLength={1200} className="bg-background" placeholder="Yard size, neighborhood, preferred timing, or property details…" /></div>
      {state === 'error' && <p role="alert" className="mt-4 text-sm font-semibold text-destructive">Something went wrong. Please try again or call us directly.</p>}
      ...
      <p className="mt-4 text-center text-sm leading-relaxed text-muted-foreground">Your details are sent to K9 Kleanup to respond to this request. No payment or commitment required.</p>
    </form>
  );
}
