...
type Options = { ok: boolean; inServiceArea: boolean; error?: string; dogs: number[]; frequencies: Array<{ value: string; label: string }>; lastTimes: Array<{ value: string; label: string }>; requiredFields?: Record<string, boolean> };
...
export function QuoteForm() {
  const [step, setStep] = useState<'zip' | 'details' | 'price' | 'finish' | 'done'>('zip');
  ...
  if (step === 'price') return <section className="rounded-2xl border bg-card p-6 sm:p-9"><p className="text-sm font-bold uppercase tracking-wider text-primary">Your Sweep &amp; Go quote</p><h3 className="mt-2 font-heading text-3xl font-black">Your yard is covered.</h3><div className="mt-6 rounded-xl bg-secondary p-5"><p className="text-sm text-muted-foreground">Per cleanup</p><strong className="mt-1 block text-4xl text-primary">{usd(price?.perCleanup) || 'Custom quote'}</strong>{price?.monthlyPrice !== null && price?.monthlyPrice !== undefined ? <p className="mt-2 text-sm text-muted-foreground">{usd(price.monthlyPrice)} estimated monthly</p> : null}</div><p className="mt-5 text-sm text-muted-foreground">Final pricing may change if yard conditions are different from the quote details.</p><Button type="button" onClick={() => setStep('finish')} className="mt-6 h-13 w-full rounded-full bg-primary text-base font-extrabold text-white hover:bg-primary/90">Start service with this quote</Button><button type="button" onClick={() => { setError(''); setStep('details'); }} className="mt-4 w-full text-sm font-bold underline">Change quote details</button></section>;
  if (step === 'finish') return <form onSubmit={(event) => { event.preventDefault(); void acceptQuote(); }} className="rounded-2xl border bg-card p-6 sm:p-9"><p className="text-sm font-bold uppercase tracking-wider text-primary">One last step</p><h3 className="mt-2 font-heading text-3xl font-black">Where should we send your service details?</h3><p className="mt-3 text-sm leading-relaxed text-muted-foreground">Your instant quote is already set. These details are only needed to create your Sweep &amp; Go service request.</p><div className="mt-6 grid gap-5 sm:grid-cols-2"><Text label="First name" id="quote-name" value={form.firstName} onChange={(value) => update('firstName', value)} /><Text label="Email" id="quote-email" type="email" value={form.email} onChange={(value) => update('email', value)} /></div><div className="mt-5"><Text label="Service address" id="quote-address" value={form.address} onChange={(value) => update('address', value)} /></div>{error ? <p role="alert" className="mt-4 text-sm font-semibold text-destructive">{error}</p> : null}<Button type="submit" disabled={busy} className="mt-6 h-13 w-full rounded-full bg-primary text-base font-extrabold text-white hover:bg-primary/90">{busy ? <><Loader2 className="size-5 animate-spin" /> Sending…</> : 'Send my service request'}</Button><button type="button" onClick={() => setStep('price')} className="mt-4 w-full text-sm font-bold underline">Back to quote</button></form>;
