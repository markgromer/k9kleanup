...
export function AdminPanel({ settings }: { settings: SiteSettings }) {
  ...
  async function saveSettings(formData: FormData) {
    ...
    } catch { setMessage('Could not connect. Please try again.'); } finally { setSaving(false); }
  }
  ...
  async function uploadImage(formData: FormData) {
    ...
    if (response.ok) window.setTimeout(() => window.location.reload(), 700);
    } catch { setMessage('Could not connect. Please try again.'); } finally { setUploading(false); }
  }
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-2">
      <form action={uploadImage} className="rounded-2xl border bg-card p-6 shadow-sm">
        ...
        <Image src={settings.heroImageKey ? `/media/${encodeURIComponent(settings.heroImageKey)}` : '/images/yard-dog.jpg'} alt="Current homepage" width={900} height={450} unoptimized={Boolean(settings.heroImageKey)} className="mt-5 aspect-[16/8] w-full rounded-xl object-cover" />
