...
const source = readFileSync(new URL('../app/api/quotes/route.ts', import.meta.url), 'utf8')
...
function handler(withDatabase = true) {
  const rows = [];
  const env = { DB: withDatabase ? { prepare: () => ({ bind: (...values) => ({ run: async () => rows.push(values) }) }) } : null };
  const context = { exports: {}, env, Response, crypto };
  vm.runInNewContext(compiled, context);
  return { post: context.exports.POST, rows };
}
const valid = { name: 'Website test', zip: '56301', email: 'test@example.com', phone: '3205550123', dogs: '2', frequency: 'weekly', notes: 'Synthetic local test only' };
const request = body => new Request('https://example.test/api/quotes', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(body) });
...
test('accepts expanded monthly and commercial choices', async () => {
  for (const frequency of ['monthly', 'commercial']) {
    const { post, rows } = handler();
    assert.equal((await post(request({ ...valid, frequency, dogs: 'shared' }))).status, 200);
    assert.equal(rows.length, 1);
  }
});
test('rejects malformed inputs without storing them', async () => {
  for (const body of [null, [], {}, { ...valid, zip: 'nope' }, { ...valid, email: 'a@' }, { ...valid, phone: '123' }, { ...valid, frequency: 'invalid' }, { ...valid, dogs: 'invalid' }]) {
    const { post, rows } = handler();
...
test('invalid JSON is handled', async () => {
  const { post } = handler();
  const req = new Request('https://example.test/api/quotes', { method: 'POST', body: '{broken' });
